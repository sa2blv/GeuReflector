...
public:
    ...
    std::vector<std::string> list_peer_host();


    void setLastTalker(int tg, const std::string& callsign) {
        last_talker[tg] = callsign;
    }

    std::string getLastTalker(int tg) {
        auto it = last_talker.find(tg);
        if (it != last_talker.end()) {
            return it->second;
        }
...
private:
    ...
    std::map<int, std::string> last_talker;  // tg -> callsign
